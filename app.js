...
async function startPresence(){if(!state.user?.id)return;await setPresence(true);if(state.presenceChannel)supabaseClient.removeChannel(state.presenceChannel);state.presenceChannel=supabaseClient.channel("profiles-presence").on("postgres_changes",{event:"UPDATE",schema:"public",table:"profiles"},payload=>{const p=payload.new;if(!p?.id)return;const i=state.profiles.findIndex(x=>String(x.id)===String(p.id));if(i>=0){state.profiles[i]={...state.profiles[i],...p};renderUsers()}else if(p.id!==state.user?.id){loadProfiles()}}).subscribe();if(state.presenceTimer)clearInterval(state.presenceTimer);state.presenceTimer=setInterval(()=>setPresence(true),2000)}
