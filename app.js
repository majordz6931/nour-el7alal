...
async function openProfile(id){
 const p=state.profiles.find(x=>String(x.id)===String(id))||state.selectedUser;
 if(!p){alert("اختر عضوًا أولًا.");return}
 const panel=$("#communityPanel");
 if(!panel){alert("تعذر فتح واجهة الملف الشخصي.");return}
 const renderProfile=profile=>{
   if(profile.avatar_path&&!profile.avatar_url){
     const {data:pub}=supabaseClient.storage.from("avatars").getPublicUrl(profile.avatar_path);
     profile.avatar_url=pub?.publicUrl||null;
   }
   const avatar=profile.avatar_url?'<img src="'+escapeHtml(profile.avatar_url)+'" alt="">':escapeHtml((profile.username||"ن")[0]);
   panel.innerHTML='<div class="panel-head"><button id="backToChat" class="ghost">← العودة للشات</button><h3>الملف الشخصي</h3><button id="closePanel" type="button">×</button></div><div class="profile-card-large profile-view"><div class="big-avatar">'+avatar+'</div><h2>'+escapeHtml(profile.username||"عضو")+(profile.verified?" ✓":"")+'</h2><p>'+escapeHtml((profile.age?profile.age+" سنة • ":"")+(profile.wilaya||"الجزائر"))+'</p><div class="profile-info-list"><div><b>الاسم الكامل</b><span>'+escapeHtml(profile.full_name||"غير محدد")+'</span></div><div><b>الجنس</b><span>'+escapeHtml(profile.gender||"غير محدد")+'</span></div><div><b>الحالة الاجتماعية</b><span>'+escapeHtml(profile.marital_status||"غير محددة")+'</span></div><div><b>المهنة</b><span>'+escapeHtml(profile.profession||"غير محددة")+'</span></div><div><b>المستوى الدراسي</b><span>'+escapeHtml(profile.education||"غير محدد")+'</span></div><div><b>يبحث عن</b><span>'+escapeHtml(profile.seeking||"غير محدد")+'</span></div><div><b>الولاية</b><span>'+escapeHtml(profile.wilaya||"الجزائر")+'</span></div><div><b>العمر</b><span>'+escapeHtml(profile.age?profile.age+" سنة":"غير محدد")+'</span></div><div><b>الحالة الآن</b><span>'+((profile.is_online&&profile.last_seen&&Date.now()-new Date(profile.last_seen).getTime()<=10000)?"🟢 متصل الآن":"⚪ غير متصل")+'</span></div><div><b>الإعجابات</b><span>'+escapeHtml(String(profile.likes_count??0))+' ❤️</span></div><div><b>نبذة</b><span>'+escapeHtml(profile.bio||"لم يكتب نبذة بعد.")+'</span></div></div><div class="profile-actions"><button class="primary" id="profileChat">💬 مراسلة</button><button class="mini-action profile-like-action" id="profileLike">❤️ إعجاب</button><button class="mini-action" id="profileRequest">💍 طلب تعارف</button><button class="mini-action" id="profileBlock">🚫 حظر</button><button class="mini-action" id="profileReport">⚠️ إبلاغ</button></div></div>';
   $("#closePanel").onclick=()=>{panel.hidden=true;const chatApp=document.querySelector(".chat-app");if(chatApp)chatApp.hidden=false};
   $("#backToChat").onclick=()=>{panel.hidden=true;const chatApp=document.querySelector(".chat-app");if(chatApp)chatApp.hidden=false};
 ...
 panel.hidden=false;
 const chatApp=document.querySelector(".chat-app");if(chatApp)chatApp.hidden=true;
 renderProfile({...p});
 try{
   const {data,error}=await supabaseClient.from("profiles").select("id,username,full_name,wilaya,age,avatar_url,avatar_path,bio,gender,marital_status,profession,education,seeking,verified,is_online,last_seen,likes_count").eq("id",id).single();
   if(!error&&data)renderProfile(data);
   else if(error)console.error("profile lookup:",error);
 }catch(err){console.error("profile lookup:",err)}
}
...
function showChatProfile(id){
  const p=state.profiles.find(x=>String(x.id)===String(id))||state.selectedUser;
  if(!p)return;
  openProfile(id).catch(err=>{console.error("profile viewer:",err);alert("تعذر فتح الملف الشخصي: "+(err.message||"خطأ غير معروف"))});
}
...
async function showChatActionsV2(id,name){
  return new Promise(resolve=>{
    closeModerationModal();
    const blocked=state.blockedIds.has(id);
    const modal=document.createElement("div");
    modal.className="moderation-modal chat-actions-modal";
    modal.innerHTML='<div class="moderation-card" role="dialog" aria-modal="true"><div class="moderation-head"><strong>إدارة العضو</strong><button type="button" class="moderation-close">×</button></div><p class="moderation-name">'+escapeHtml(name||"العضو")+'</p><select class="moderation-select" id="chatActionsSelect"><option value="">اختر الإجراء...</option><option value="block">'+(blocked?"🔓 فك الحظر":"🚫 حظر العضو")+'</option><option value="report">⚠️ إبلاغ عن العضو</option></select><div class="moderation-footer"><button type="button" class="moderation-cancel">إلغاء</button><button type="button" class="moderation-confirm">متابعة</button></div></div>';
    document.body.appendChild(modal);
    const finish=a=>{modal.remove();resolve(a)};
    modal.querySelector(".moderation-close").onclick=()=>finish(null);
    modal.querySelector(".moderation-cancel").onclick=()=>finish(null);
    modal.onclick=e=>{if(e.target===modal)finish(null)};
    modal.querySelector(".moderation-confirm").onclick=()=>finish(modal.querySelector("#chatActionsSelect").value||null);
  });
}
function showModerationMenu(id,name){
