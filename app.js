...
function playMessageSound(){try{const C=window.AudioContext||window.webkitAudioContext;if(!C)return;const ctx=window.__nourAudio||(window.__nourAudio=new C());if(ctx.state==="suspended")ctx.resume();const now=ctx.currentTime;[0,0.14].forEach((d,i)=>{const o=ctx.createOscillator(),g=ctx.createGain();o.type="sine";o.frequency.setValueAtTime(i?880:660,now+d);g.gain.setValueAtTime(0.0001,now+d);g.gain.exponentialRampToValueAtTime(0.12,now+d+0.015);g.gain.exponentialRampToValueAtTime(0.0001,now+d+0.12);o.connect(g);g.connect(ctx.destination);o.start(now+d);o.stop(now+d+0.13)})}catch(e){console.debug("message sound:",e)}}\nfunction notifyMessage(message){playMessageSound();const sender=state.profiles.find(p=>p.id===message.sender_id);const name=sender?.username||"عضو جديد";const text=String(message.body||"رسالة جديدة").slice(0,120);if(document.hidden&&"Notification" in window&&Notification.permission==="granted"){new Notification("نور الحلال • "+name,{body:text,icon:sender?.avatar_url||undefined,tag:"message-"+message.id})}if(navigator.vibrate)navigator.vibrate([120,60,120])}
